...
public class SimulationHandler {
    // carbon from ionization scheme -> loss scheme : charge + 1
    public static final Particle.ParticleName carbonIon = Particle.ParticleName.CARBON5P;
    ...
    private static int binsEb = 1;
    private static final int simulationNumber = 100;
    private double KE = 0;
    ...
    public static RecGenerator.Orbital getOrbital(int index) {
        ...
        switch (indexEb) {
            ...
            case 5:
                return RecGenerator.Orbital.EB6;
        }
        return RecGenerator.Orbital.EB1;
    }
    ...
    public void analyzeOneJob(int index, CrossSection crossSection, ArrayList<Outcome> outcomes) {
        ...
        if (getIndexEb(index) % 2 == 0) {
            Psi = impactParameter*(Pion)*delta_b;
        } else {
    ...
    public static void testRec() throws IOException, NullPointerException{
        ...
        for (RecGenerator.Orbital orb : RecGenerator.Orbital.values()) {
            RecGenerator r_gen = new RecGenerator(orb, carbonIon);
    ...
    public void testOneSimulation() {
        // Test one simulation
        RecGenerator r_gen = new RecGenerator(RecGenerator.Orbital.EB1, carbonIon);
        ...
        double b = 0.7;
        LossScheme lossScheme = new LossScheme(RecGenerator.Orbital.EB1, KE, b, carbonIon);
        lossScheme.initialize(r_gen, rand1, rand2, rand3);
        System.out.println(lossScheme.toString());
        double time = lossScheme.RungeKuttaABC();
        System.out.println(lossScheme.toString());
        System.out.println("time = " + time);
        System.out.println("Eet " + lossScheme.Eet());
        System.out.println("Eep " + lossScheme.Eep());
        System.out.println("theta " + lossScheme.theta());
        System.out.println("theta " + lossScheme.calcAzimuthalAngle());
        System.out.println("outcome " + lossScheme.getOutcome());
    }
